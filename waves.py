...
#Задаем константы и параметры по умолчанию
...
class mywindow(QtWidgets.QMainWindow):
    ...
    def btnClicked(self):
        try:
            ...
            if w==0 or w<wkr:
                    raise Exception("Частота должна быть больше критической частоты")
            ...
            if str(self.ui.comboBox.currentText())=="ТЕ":
                ...
                fig= plt.figure("Графики TE-поляризации", figsize=(8,6))
                ...
                #     ims.append([YZE])
                fig= plt.figure("Графики для TM-поляризации", figsize=(8,6))
                gs= gridspec.GridSpec(nrows=3,ncols=2)
                fig.suptitle('TM волна моды {}{}'.format(m, n), fontsize=12)
                #YZ  
                # #E
